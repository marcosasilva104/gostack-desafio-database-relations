...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const exixtentProductsIds = existentProducts.map(product => product.id);

    const checkInexistentProducts = products.filter(
      product => !exixtentProductsIds.includes(product.id),
    );
    ...
    if (findProductWithNoQuantityAvaliable.length) {
      throw new AppError(
        `The quantity ${findProductWithNoQuantityAvaliable[0].quantity} is not available for ${findProductWithNoQuantityAvaliable[0].id}`,
      );
    }
